...
/*   Updated: 2024/08/26 10:19:28 by mafurnic         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_plane_color(char *line, int *i, t_plane *plane,t_scene *scene)
{
	...
	r = parse_color_component(line, i, "Invalid character in plane definition", scene);
	skip_whitespace(line, i);
	if (line[*i] == ',')
		(*i)++;
	skip_whitespace(line, i);
	g = parse_color_component(line, i, "Invalid character in plane definition", scene );
	if (line[*i] == ',')
		(*i)++;
	skip_whitespace(line, i);
	b = parse_color_component(line, i, "Invalid character in plane definition", scene);
